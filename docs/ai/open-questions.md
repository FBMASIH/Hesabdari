...
## OQ-008 — Opening balance date fields
`BankOpeningBalance` and `CashboxOpeningBalance` have `date` field in schema but the contract uses `balanceDate`. Need to verify alignment.
