...
export function DataFilterBar({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 pb-4">
      {/* Search */}
      <div className="flex h-9 flex-1 min-w-[200px] max-w-sm items-center gap-2 rounded-xl border border-border-primary bg-bg-primary/60 px-3">
      ...
      {filters?.map((f) => (
        <button
          ...
          className={cn('flex h-9 items-center rounded-xl border px-3 text-xs font-medium transition-colors',
            f.active
