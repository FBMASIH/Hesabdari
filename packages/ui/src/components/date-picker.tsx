...
interface JDate { jy: number; jm: number; jd: number }
...
function toJalali(date: Date): JDate {
  ...
  let days = 355666 + 365 * gy + Math.floor((gy2 + 3) / 4) - Math.floor((gy2 + 99) / 100) + Math.floor((gy2 + 399) / 400) + gd + (gDays[gm - 1] ?? 0);
...
function fromJalali(jy: number, jm: number, jd: number): Date {
  let gy: number, adj = jy;
  if (adj > 979) { gy = 1600; adj -= 979; } else { gy = 621; }
  let days = 365 * adj + Math.floor(adj / 33) * 8 + Math.floor((adj % 33 + 3) / 4) + 78 + jd + (jm < 7 ? (jm - 1) * 31 : (jm - 7) * 30 + 186);
  ...
  if (gm === 0) {
    ...
    for (gm = 3; gm <= 12; gm++) { if (rem < (md[gm] ?? 0)) break; rem -= md[gm] ?? 0; }
    days = rem;
  } else { days = gm === 2 ? days - 31 : days; }
...
export function DatePicker({ className, value, onChange, hasError, placeholder = '\u06F1\u06F4\u06F0\u06F5/\u06F0\u06F1/\u06F0\u06F1', disabled, ...props }: DatePickerProps) {
  ...
  useEffect(() => {
    if (value) {
      ...
      if (d) {
        const j = toJalali(d);
        setInputText(`${toPersian(String(j.jy))}/${toPersian(String(j.jm).padStart(2, '0'))}/${toPersian(String(j.jd).padStart(2, '0'))}`);
        return;
      }
    }
    setInputText('');
  ...
  const handleInputBlur = useCallback(() => {
    if (!inputText.trim()) { onChange(''); return; }
  ...
  const isSelected = (d: number) => sel && sel.jy === viewYear && sel.jm === viewMonth && sel.jd === d;
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {/* Dropdown calendar */}
      {open && !disabled && (
        <div className="absolute top-[calc(100%+4px)] start-0 z-50 w-[260px] rounded-lg border border-border-primary bg-bg-primary p-3 shadow-lg" dir="rtl">
          {/* Header: nav arrows + month/year */}
          <div className="mb-2 flex items-center justify-between">
            <button type="button" onClick={nextMonth} className="rounded p-1 text-fg-secondary hover:bg-bg-tertiary">
              <IconChevronRight size={16} />
            </button>
            <span className="text-[13px] font-semibold text-fg-primary">
              {MONTH_NAMES[viewMonth - 1]} {toPersian(viewYear)}
            </span>
            <button type="button" onClick={prevMonth} className="rounded p-1 text-fg-secondary hover:bg-bg-tertiary">
              <IconChevronLeft size={16} />
            </button>
          </div>

          {/* Weekday labels */}
          <div className="mb-1 grid grid-cols-7 text-center text-[11px] text-fg-tertiary">
